...
- [x] T115 [P] [US7] RED: create `ios/Tests/ImportAccessibilityTests.swift` covering every screen in the flow — largest accessibility Dynamic Type with nothing clipped or unreachable, Dark Mode, Reduce Transparency and Increase Contrast holding contrast, and VoiceOver announcing every control, count, amount and warning with no unlabelled element (FR-044, FR-046, SC-009). **Settled:** split in two, because a unit target cannot see a rendered screen. `ios/Tests/ImportAccessibilityTests.swift` pins the half that is provable without a UI: every failure, verdict and stage has its own sentence and its own symbol, the empty state keeps its privacy promise, and the front door follows what has actually been imported. A new **`KanameUITests`** target (`ios/UITests/`) runs the system's own `performAccessibilityAudit()` against the front door at default and at the largest accessibility text size. The screens behind an import would need test-only entry points in the shipped app to reach from XCUITest, so they stay with the manual gate (T123). **The audit earned its place immediately** — it found clipped text and four real contrast failures: `.foregroundStyle(.secondary)` does not clear the contrast threshold at any size used here, `LabeledContent` renders its value in that same secondary style (so every figure now sets `.primary` explicitly), and a `.glassProminent` button refracting scrolled text fails at accessibility text sizes, which is why both bottom action bars sit on an opaque background. Kaname also gained its own accent colour (`ios/Sources/Theme.swift`), a deep ink-teal clearing 4.5:1 against white in both appearances.

### Implementation for User Story 7

- [x] T116 [US7] Create `ios/Sources/Import/ImportEmptyStateView.swift`: what Kaname does, the "your data stays on this device" promise, and a single `Button(…).buttonStyle(.glassProminent)` in `.safeAreaInset(edge: .bottom)` — the screen's one prominent element (FR-039, R13).
- [x] T117 [US7] Update `ios/Sources/RootView.swift` to switch between the empty state and the imported accounts once at least one import exists, keeping the Import action reachable from both (FR-040, US7 §3).
- [x] T118 [US7] Confirm the primary action opens the system document picker in **one** tap in `ios/Sources/RootView.swift`, keeping the whole path within the 4-tap budget (SC-001, US7 §2).
- [x] T119 [US7] Audit every count and amount across `ios/Sources/Import/` for `.monospacedDigit()` (FR-045).
- [x] T120 [US7] Review every view under `ios/Sources/Import/` against `.github/skills/swiftui-liquid-glass/SKILL.md`: at most one tinted/prominent element per screen, no glass on dense rows, system chrome not re-skinned, no debit/credit colour signal on a tinted surface (FR-047).
- [x] T121 [US7] Extend `scripts/import-path-audit.sh` with a hard failure on `#available(iOS 26`, `.ultraThinMaterial`, `UIVisualEffectView` or any hand-rolled blur anywhere under `ios/Sources/`.
- [x] T122 [US7] **GATE** `make lint && make ios-test && make import-audit` — T115 GREEN. Targets live in the repo-root `Makefile`.
...
- [ ] T129 Run the manual smoke test in `specs/016-statement-import-vertical/quickstart.md` §5: the 4-tap path, force-quit and relaunch, the same-file re-import, then the full failure matrix (image-only, password right and wrong, corrupt, `.txt` renamed `.pdf`, a utility bill, cancel mid-parse). **First run done — three findings, one fixed; see `.scratch/HANDOFF.md` § "T129 — first run".** The 4-tap path, force-quit/relaunch and the failure matrix behaved. The re-import did not: the accounts list counted superseded and deleted rows, so a person's history visibly doubled on the one screen FR-025 promises it will not — fixed in `3ba7890`, and the live-row predicate now lives on `StoredTransaction.isLive` for every P3 screen that follows. Two questions remain open and block the tick: a corrupt PDF and a `.txt` renamed `.pdf` produce the *same* sentence (any nil `PDFDocument` is `.notAPDF`), and the summary's `transactionsImported` counts rows written rather than rows kept, so a re-import reads "1 imported, 1 duplicate skipped".
